...
const features = [
  {
    ...
    description:
      "Keep every member organized — profiles, activity history, and account balances all in one place. No more hunting through folders.",
    icon: (
      <svg
        viewBox="0 0 24 24"
        fill="none"
    ...
    preview: (
      <div className="mt-4 space-y-1.5">
        {[
          "Ana Garcia — ₱12,400 savings",
          "Juan Cruz — Loan active",
          "Maria Reyes — ₱8,200 savings",
        ].map((t, i) => (
          <div
            key={i}
            className="flex items-center gap-2 rounded-md bg-surface/60 px-2.5 py-1.5 text-[10px] text-muted-foreground"
          >
  ...
  },
  {
    title: "Loan Tracking",
    description:
      "See every loan from application to final payment. Automated reminders keep borrowers on track and your records clean.",
    icon: (
      <svg
        viewBox="0 0 24 24"
        fill="none"
    ...
    preview: (
      <div className="mt-4 flex items-center gap-3">
        ...
        <div className="text-right">
          <p className="text-xs font-semibold text-foreground">₱180K</p>
  ...
  },
  {
    ...
    description:
      "Watch individual and pooled savings grow in real time. Automate contribution schedules so nothing slips through the cracks.",
    icon: (
      <svg
        viewBox="0 0 24 24"
        fill="none"
    ...
    preview: (
      <div className="mt-4 flex items-end gap-[3px] h-10">
        {[30, 45, 38, 55, 48, 62, 58, 75, 68, 84, 78, 90].map((h, i) => (
          <div
            key={i}
            className="flex-1 rounded-sm bg-gradient-to-t from-primary/50 to-accent/30"
            style={{ height: `${h}%` }}
          />
        ))}
      </div>
  ...
  },
  {
    ...
    preview: (
      <div className="mt-4 grid grid-cols-2 gap-1.5">
        {["Balance Sheet", "Cash Flow", "Loan Aging", "Member Equity"].map(
          (r) => (
            <div
              key={r}
              className="rounded-md bg-surface/60 px-2 py-1.5 text-[9px] text-muted-foreground text-center"
            >
              {r}
            </div>
          ),
      ...
      <div className="mx-auto max-w-7xl px-4 pt-16 sm:px-6 lg:px-8">
        <FadeIn className="mx-auto max-w-2xl text-center">
          ...
          <h2 className="text-3xl font-bold tracking-tight sm:text-4xl">
            ...
            <span className="gradient-text">how cooperatives work</span>
          </h2>
          <p className="mt-3 text-base leading-relaxed text-muted-foreground">
            Not another generic finance app. Every feature was designed around
            the workflows cooperatives deal with every day.
          </p>
        </FadeIn>
        ...
        >
          {features.map((f) => (
            <StaggerItem key={f.title}>
              <motion.div
                className="group relative flex h-full flex-col rounded-xl border border-border bg-card p-6 transition-all duration-300 hover:border-primary/20 hover:shadow-lg hover:shadow-primary/[0.04]"
                whileHover={{ y: -4 }}
