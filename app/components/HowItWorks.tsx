...
const steps = [
  {
    ...
    description:
      "Import from a spreadsheet or add people one by one. Each member gets their own secure portal within minutes.",
    icon: (
  ...
  },
  {
    ...
    description:
      "Track dues, record deposits, process loan requests, and schedule reminders. The busy-work runs on autopilot.",
    icon: (
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="w-6 h-6">
  ...
  },
  {
    ...
    description:
      "One-click reports for your board, auditors, and members. Know exactly where things stand, at any time.",
    icon: (
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="w-6 h-6">
  ...
  return (
    <div ref={ref} className="hidden lg:flex items-center justify-center px-2">
      ...
      >
        ...
        <svg className="h-3 w-3 -ml-0.5 text-primary/30" viewBox="0 0 12 12" fill="currentColor">
...
export default function HowItWorks() {
  return (
    <section id="how-it-works" className="relative py-24 sm:py-32 bg-surface/40">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-14 grid items-start gap-6 lg:grid-cols-[1fr_auto_1fr_auto_1fr] lg:gap-0">
          {steps.map((step, i) => (
            <>
              <FadeIn key={step.title} delay={0.1 + i * 0.12} className="relative">
                <motion.div
              ...
              {i < steps.length - 1 && <ConnectorLine key={`line-${i}`} index={i} />}
            </>
          ))}
        </div>
      </div>
